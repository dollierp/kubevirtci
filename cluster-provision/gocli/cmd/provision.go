...
func provisionCluster(cmd *cobra.Command, args []string) (retErr error) {
	...
	if centosVersion == "" {
		...
		if err != nil {
			return fmt.Errorf("reading base file for provider %q: %w", packagePath, err)
		}
	...
	if strings.Contains(phases, "linux") {
		base = fmt.Sprintf("quay.io/dollierp/centos%s", centosVersion)
	} else {
	...
	target := fmt.Sprintf("quay.io/dollierp/k8s-%s", name)
